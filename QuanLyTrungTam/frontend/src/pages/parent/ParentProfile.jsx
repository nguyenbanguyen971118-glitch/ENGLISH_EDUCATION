...
// Màn hồ sơ phụ huynh dùng API thật, có xem/sửa thông tin và quản lý avatar
// theo cùng pattern với StudentProfile/TeacherProfile.
const EMPTY_PROFILE = {
    HoTen: '',
    Email: '',
    AnhDaiDien: '',
...
const FIELD_GROUPS = [
    ...
    {
        title: 'Liên hệ và nghề nghiệp',
        icon: 'bi-briefcase',
        fields: [
            { name: 'NgheNghiep', label: 'Nghề nghiệp', placeholder: 'Nhập nghề nghiệp' },
            { name: 'DiaChiLienHe', label: 'Địa chỉ liên hệ', placeholder: 'Nhập địa chỉ liên hệ', type: 'textarea' }
        ]
    }
];

const OPTIONAL_PROFILE_FIELDS = ['DiaChiLienHe', 'NgheNghiep'];
...
const normalizeProfile = (profile) => ({
    ...
    DiaChiLienHe: profile?.DiaChiLienHe || profile?.diaChiLienHe || '',
    NgheNghiep: profile?.NgheNghiep || profile?.ngheNghiep || ''
});
...
const ParentProfile = () => {
    ...
    const syncAuthUser = (nextProfile) => {
        ...
        updateUser((prevUser) => ({
            ...
            avatar: parentService.resolveStoredFileUrl(normalizedNextProfile.AnhDaiDien) || prevUser?.avatar || '',
    ...
    useEffect(() => {
        ...
        // Tải profile thật từ backend thay cho dữ liệu mock/localStorage cũ.
        const fetchProfile = async () => {
            try {
                const data = normalizeProfile(await parentService.getProfile());
                if (!isMounted) {
                    return;
                }
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (selectedAvatarFile) {
                const uploadResult = await parentService.uploadAvatar(selectedAvatarFile);
                nextFormData = {
                    ...nextFormData,
            ...
            const updatedProfile = normalizeProfile(await parentService.updateProfile(nextFormData));
    ...
    const handleAvatarUpload = async () => {
        ...
        try {
            ...
            const uploadResult = await parentService.uploadAvatar(file);
    ...
    const handleDeleteAvatar = async () => {
        ...
        try {
            ...
            await parentService.deleteAvatar();
    ...
    // Reset nhóm thông tin mở rộng của phụ huynh, vẫn giữ lại họ tên/email/số điện thoại.
    const handleResetOptionalFields = async () => {
        if (!window.confirm('Bạn có chắc muốn đặt lại các thông tin mở rộng không?')) {
            return;
        }
        ...
        try {
            ...
            const updatedProfile = normalizeProfile(await parentService.updateProfile(resetData));
            setProfile(updatedProfile);
            setFormData(updatedProfile);
            syncAuthUser(updatedProfile);
            setSuccessMessage('Đã đặt lại thông tin mở rộng.');
        } catch (err) {
            setSaveError(err.message || 'Không thể đặt lại thông tin.');
        } finally {
    ...
    const avatarSrc =
        previewImage ||
        parentService.resolveStoredFileUrl(formData.AnhDaiDien) ||
        `https://ui-avatars.com/api/?name=${encodeURIComponent(formData.HoTen || 'Parent')}&background=005197&color=ffffff&bold=true&size=256`;
        ...
        return (
            <div className="d-flex align-items-center justify-content-center" style={{ minHeight: '60vh' }}>
                <div className="text-center">
                    ...
                    <div className="text-muted fw-semibold">Đang tải thông tin phụ huynh...</div>
                </div>
            </div>
        );
    }
                                ...
                                <div className="d-flex align-items-center gap-4 flex-wrap">
                                    ...
                                    <div>
                                        ...
                                        <h1 className="mb-2 fw-bold text-dark" style={{ fontSize: '2rem' }}>
                                            {profile.HoTen || 'Phụ huynh English Education'}
                                        </h1>
                                        <div className="d-flex flex-wrap gap-2 mb-3">
                                            ...
                                            <span className="badge rounded-pill bg-white text-dark border px-3 py-2">
                                                {profile.NgheNghiep || 'Chưa cập nhật nghề nghiệp'}
                                            </span>
                                        </div>
                                        <div className="d-flex flex-wrap gap-4 text-secondary small fw-semibold">
                                            ...
                                            <span>
                                                ...
                                                {profile.SoDienThoai || 'Chưa cập nhật số điện thoại'}
                                            </span>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div className="col-sm-6">
                                        <div className="profile-stat-card h-100">
                                            ...
                                            <div className="fw-bold text-dark">
                                                {profile.NgheNghiep || 'Chưa cập nhật'}
                                            </div>
                                        </div>
                                    </div>
                                    <div className="col-sm-6">
                                        <div className="profile-stat-card h-100">
                                            ...
                                            <div className="fw-bold text-dark">
                                                {profile.DiaChiLienHe || 'Chưa cập nhật'}
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                        ...
                                        {!isEditing ? (
                                            ...
                                            <>
                                                ...
                                                >
                                                    ...
                                                    {isResettingOptional ? 'Đang đặt lại...' : 'Đặt lại thông tin mở rộng'}
                                                </button>
                                                <button
                                                    type="button"
                                            ...
                                            <div className="d-flex align-items-center gap-3 mb-4">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-muted small mb-0">
                                                        Ảnh được upload trực tiếp lên server và lưu cho tài khoản phụ huynh.
                                                    </p>
                                                </div>
                                            </div>
                        ...
                        <div className="card border-0 rounded-5 shadow-sm overflow-hidden mb-4">
                            <div className="card-body p-4">
                                ...
                                <p className="text-muted mb-4">
                                    Hồ sơ này được dùng để hiển thị thông tin phụ huynh trên hệ thống, tin nhắn và các màn hình liên quan.
                                </p>
                                ...
                                <div className="d-grid gap-3">
                                    ...
                                    <div className="profile-mini-info">
                                        <span className="text-muted small">Nghề nghiệp</span>
                                        <strong className="text-dark">{profile.NgheNghiep || 'Đang bổ sung'}</strong>
                                    </div>
                                    <div className="profile-mini-info">
                                        <span className="text-muted small">Địa chỉ liên hệ</span>
                                        <strong className="text-dark">{profile.DiaChiLienHe || 'Đang bổ sung'}</strong>
                                    </div>
                                </div>
                            </div>
                        </div>
