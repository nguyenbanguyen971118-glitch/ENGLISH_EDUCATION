...
// Service gom các API hồ sơ giảng viên để page profile và luồng login dùng chung.
...
const teacherService = {
    // Lấy profile thật của giảng viên đang đăng nhập.
    getProfile: async () => {
        const response = ensureSuccess(
            await apiClient.get('/GiangVien/my-profile'),
    ...
    updateProfile: async (profileData) => {
        const response = ensureSuccess(
            await apiClient.put('/GiangVien/update-profile', profileData),
    ...
    uploadAvatar: async (file) => {
        ...
        const response = await fetch(`${getApiBaseUrl()}/GiangVien/upload-avatar`, {
            method: 'POST',
    ...
    // Xóa avatar hiện tại của giảng viên.
    deleteAvatar: async () => {
        const response = ensureSuccess(
            await apiClient.delete('/GiangVien/delete-avatar'),
